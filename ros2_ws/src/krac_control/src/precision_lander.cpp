...
private:
    ...
    const double CLOSE_RANGE_LOCK_ALT_M = 2.0;
    const double ALIGN_GRACE_SEC = 4.0;
    ...
    void control_loop() {
        ...
        if (!target_fresh) {
            const bool recently_aligned =
                has_been_aligned_ &&
                (this->now() - last_aligned_time_).seconds() <= ALIGN_GRACE_SEC;

            if (current_alt_ <= CLOSE_RANGE_LOCK_ALT_M && recently_aligned) {
                vel_cmd.linear.x = 0.0;
                vel_cmd.linear.y = 0.0;
                vel_cmd.linear.z = PRECISION_DESCEND_SPEED;
                vel_cmd.angular.z = 0.0;
                RCLCPP_WARN_THROTTLE(this->get_logger(), *this->get_clock(), 1000,
                    "[Target LOST near ground, alt=%.2fm] trusting last known alignment, "
                    "continuing blind descent instead of aborting.", current_alt_);
            ...
            vel_cmd.angular.z = 0.0;
            RCLCPP_WARN_THROTTLE(this->get_logger(), *this->get_clock(), 1000,
                "[Target LOST] holding position; BT recovery must reacquire before descent.");
            vel_pub_->publish(vel_cmd);
            return;
        }
        ...
        vel_cmd.linear.x = body_vx * cos_yaw - body_vy * sin_yaw;
        vel_cmd.linear.y = body_vx * sin_yaw + body_vy * cos_yaw;
