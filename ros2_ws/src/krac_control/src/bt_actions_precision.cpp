...
{
  return {BT::InputPort<int>("seq", 0, ""), BT::InputPort<double>("timeout_sec", 300.0, "")};
}
BT::NodeStatus WaitForWaypointReached::onStart()
{
  ...
  if (ctx_->lastReachedWaypointSeq() >= seq_) return BT::NodeStatus::SUCCESS;
...
BT::PortsList PrecisionLandOnTarget::providedPorts()
{
  return {
    ...
    BT::InputPort<double>("target_fresh_max_age_sec", 1.0, "")
  };
}
...
BT::NodeStatus OpenGripper::onStart()
{
  ...
  RCLCPP_INFO(ctx_->node()->get_logger(), "Opening gripper via %s=%.2f, %s=%.2f",
              left_topic_.c_str(), left_position_, right_topic_.c_str(), right_position_);
  return BT::NodeStatus::RUNNING;
}
